...
export const api = {
  ...
  splits: {
    ...
    update: async (splitId: string, creatorId: string, updates: { people_needed?: number; split_time?: string; time_note?: string }): Promise<GenericResponse<MealSplit>> => {
      try {
        // Verify caller is the creator
        const { data: split } = await supabase.from('meal_splits').select('creator_id').eq('id', splitId).single();
        if (!split || split.creator_id !== creatorId) {
          return { success: false, message: 'Only the creator can edit this split.' };
        }

        // Validate people_needed is not less than current participants
        if (updates.people_needed) {
          const currentCount = await getParticipantCount(splitId);
          if (updates.people_needed < currentCount) {
            return { success: false, message: `Cannot reduce below ${currentCount} (current participants).` };
          }
        }
        ...
        if (error) throw error;
        const enriched = await enrichSplitsWithParticipants([data]);
        return { success: true, message: 'Split updated!', data: enriched[0] };
      } catch (error: any) {
        return { success: false, message: error.message };
      }
    }
